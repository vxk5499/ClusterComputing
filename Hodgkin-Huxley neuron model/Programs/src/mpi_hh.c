...
int main( int argc, char **argv )
{
  ...
  int k,l,m,n,u,v;						 // Values declared to measure start and end of loop as well as number of dendrites processed by the processors and remainder.

  MPI_Comm_rank(MPI_COMM_WORLD, &rank); //Finds the rank of processors
  MPI_Comm_size(MPI_COMM_WORLD,&size); // Finds the number of processors

  double exec_time,s;  // How long we take, temporary variable
  ...
  // Loop over milliseconds.
	...
	for (step = 0; step < STEPS; step++) {
			...
			//master sends y[0] to all slaves
	 if(rank==0){				   
					for(j=1;j<size;j++)
					MPI_Send(&y[0],1,MPI_DOUBLE,j,tag,MPI_COMM_WORLD);
						//printf("\r%02d send",rank);
				}
	  // Loop over all the dendrites.
	  //the number of dentrites is equal to the number of slaves
	   if(num_dendrs==(size-1))
		{
			 if(rank!=0)
			 {
				//printf("\r%02d recv",rank);
				MPI_Recv(&y[0],1,MPI_DOUBLE,0,tag,MPI_COMM_WORLD,&status);//the slaves receive the y[0] value
		  
				
				// This will update Vm in all compartments and will give a new injected
				// current value from last compartment into the soma.
				//each slave calulates the current in one dendrite
				current = dendriteStep( dendr_volt[ rank-1 ],
									step + (rank-1) + 1,
									num_comps,
									soma_params[0],
									y[0] );
				...
				//the calculated current is passed on to the master
				MPI_Send(&soma_params[2],1,MPI_DOUBLE,0,tag,MPI_COMM_WORLD);
					//printf("\r%02d send",rank);
			}  
			else
			{
				//the master accumulates all the current from the slaves to calculate the final output
				for( j=1;j<size;j++)
				{
					s=0;
					MPI_Recv(&s,1,MPI_DOUBLE,MPI_ANY_SOURCE,tag,MPI_COMM_WORLD,&status);
					//printf("\r%02d recv",rank);
					soma_params[2]+=s;
				}
			}
		}
		//if the number of dendrites is more than the number of slaves
		else{
		
			k=num_dendrs/size;	//indicates the number of dendrites to be processed by each processor
			n=num_dendrs%size;	//indicates the number of extra dendrites to be processed by the master 
			u=rank*k;			//indicates the starting dendrite number for the respective processor
			v=(rank+1)*k-1;		//indicates the ending dendrite number for the respective processor
			if (rank !=0)
			MPI_Recv(&y[0],1,MPI_DOUBLE,0,tag,MPI_COMM_WORLD,&status);
			
			  for (dendrite = u; dendrite <= v; dendrite++) 
			  {
					// This will update Vm in all compartments and will give a new injected
					// current value from last compartment into the soma.
			  ...
			  //soma param values are sent to master from slaves
				if (rank !=0)
				MPI_Send(&soma_params[2],1,MPI_DOUBLE,0,tag,MPI_COMM_WORLD);
				//soma param values are received by master from slaves
			   if(rank==0)
				{
					for(j=1;j<size;j++)
					{
		...
		//if there are unprocessed dendrites then the master would process them
	    if (n!=0 && rank==0)
		
	  {
	    u=k*size;		// starting dendrite number for processing
		
			  for (dendrite = u; dendrite < num_dendrs; dendrite++) {
			// This will update Vm in all compartments and will give a new injected
			// current value from last compartment into the soma.
			current = dendriteStep( dendr_volt[ dendrite ],
									step + dendrite + 1,
  ...
  free(dendr_volt);
   ...
   MPI_Finalize();		//Indicates the end of MPI implementation 
